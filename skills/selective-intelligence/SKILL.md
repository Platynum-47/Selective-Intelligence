...
These gates bind only when enforced — by the eval suite, the SI Python runtime checkpoint/interrupt path, and the product that wires those checks before the user ever sees drifted work — not by this prose alone. Documented is not enforced. Read the honest runtime map in [references/step1-intent-control-status.md](references/step1-intent-control-status.md): Step-1 P0 ops 1–4 are enforced in SI runtime tests; Platynum live-steering UI (PR #2) is merged surface only until 👎 calls SI interrupt; cross-model equivalence remains unproven.
